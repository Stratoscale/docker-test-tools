...
from docker_test_tools import environment, compose
...
"""
    ...
    def test_container_methods_happy_flow(self):
        ...
        with mock.patch.object(subprocess, "check_output") as mock_command:
            mock_command.return_value = "container-id"
        ...
        with mock.patch(
            "docker_test_tools.compose.Compose.get_service_container_id",
            ...
            with mock.patch.object(
                docker.APIClient, "inspect_container"
            ) as mock_inspect:
    ...
    @mock.patch(
        ...
        mock.MagicMock(side_effect=subprocess.CalledProcessError(1, "", "")),
    )
    def test_environment_compose_command_error(self):
        ...
        with mock.patch(
            "docker_test_tools.environment.EnvironmentController.get_container_id",
            mock_get_id,
        ):
            self.controller.docker_client.inspect_container = mock.MagicMock(
                return_value={"State": {"Status": "not-running"}}
            )
            self.assertFalse(self.controller.is_container_ready("test"))
    ...
    def test_container_methods_bad_service_name(self):
        ...
        with mock.patch.object(
            compose.Compose, "get_service_container_id"
        ) as mock_compose:
            mock_compose.side_effect = RuntimeError("test-error")
            ...
            self.assertFalse(self.controller.is_container_ready(service_name))

    @mock.patch(
        "docker_test_tools.environment.EnvironmentController.is_container_ready"
    )
    ...
    @mock.patch(
        "docker_test_tools.environment.EnvironmentController.get_services",
        mock.MagicMock(),
    )
    @mock.patch("docker_test_tools.environment.EnvironmentController.down")
    ...
    def test_wait_for_services(self, mock_is_container_ready):
        ...
        mock_is_container_ready.assert_has_calls(
            [mock.call("service1"), mock.call("service2")], any_order=True
        )
        mock_is_container_ready.side_effect = TimeoutExpired(
            timeout_seconds=1, what="something"
        )
    ...
    def test_from_file(self):
        ...
        with mock.patch("subprocess.check_output", return_value="service1\nservice2\n"):
            with mock.patch(
                "docker_test_tools.config.Config", return_value=mocked_config
            ):
    ...
    @mock.patch(
        "docker_test_tools.environment.EnvironmentController._get_environment_variables",
        mock.MagicMock(return_value=ENVIRONMENT_VARIABLES),
    )
    def get_controller(self):
        """Returns a new EnvironmentController."""
        with mock.patch("subprocess.check_output", return_value="service1\nservice2\n"):
            return environment.EnvironmentController(
                log_path=self.log_path,
                compose_path=self.compose_path,
                project_name=self.project_name,
                compose_command="docker-compose",
